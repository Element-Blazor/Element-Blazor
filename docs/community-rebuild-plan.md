...
| 任务 | 状态 | 验收 |
| --- | --- | --- |
| 明确旧 `community` 为素材库，不作为兼容升级目标 | 🟢 已完成 | 文档写明不保留旧组件名、旧包名、旧启动方式。 |
| 选定首阶段技术形态 | 🟢 已完成 | 推荐 Blazor Server/Web App + EF Core + SQLite 本地基线。 |
| 建立新项目命名 | 🟢 已完成 | `ElementCommunity.*`，避免继续使用 `BlazorCommunity` 历史包名。 |
| 冻结旧项目改动 | 🟢 已完成 | 旧项目只读参考，不继续修小问题。 |
...
1. 🟢 已完成 C0-1：在仓库中冻结旧 `community`，把它标为参考素材。
